...
> **注意**：后端当前为 Mock 实现，所有 API 返回模拟数据。真实业务逻辑待接入 OpenSearch/Neptune/Bedrock。
...
**技术实现**：
- ~~基于 Amazon Neptune 图数据库~~（Mock 实现）
- ~~支持 Gremlin 查询语言~~（待实现）
- ~~动态布局算法~~（待实现）
...
| `/graph` | 知识图谱 | ✓ | ✅ Mock |
| `/documents` | 文档管理 | ✓ | ✅ Mock |
| `/services` | 服务状态 | ✓ (底部) | ✅ Mock |
| `/monitoring` | 监控面板 | ✓ (底部) | ✅ 完成 |
| `/about` | 关于 | ✓ (底部) | ✅ 完成 |
| `/design-system` | 设计系统 | 快速操作入口 | ✅ 完成 |
